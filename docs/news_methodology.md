...
News updates are manual by default:

```bash
python3 scripts/update_news.py
```

That command creates or appends to the digest for the day it is run. Each date contains separate buckets for every required GP: Blue Owl, OTF, Pretium, KKR, PAG, Bayview, CIFC, Basepoint, NB, Apollo, Bain Capital, Guggenheim, and HSBC AM. It also prunes digest dates older than the configured rolling window, currently 90 days. To automate it, schedule the same command with cron, launchd, GitHub Actions, or another job runner at the preferred time, for example once each Hong Kong morning after the US market close.
...
Each fetched item is scored using transparent keyword rules:
...
The script then sorts by score and date inside each GP bucket. This avoids compressing a day into one short global list and lets the selected date show different sources for each manager on the same screen.
...
- The updater creates English summaries from article titles and RSS snippets. The seeded digest includes manually prepared Traditional Chinese summaries; future machine-updated Chinese summaries are marked for review unless a translation / summarization service is added.
- Keyword scoring is transparent but imperfect. It can miss nuanced relevance, especially where a mega-manager appears in a non-credit article that still has credit read-through.
...
- No alerting, email delivery, authentication, or analyst approval queue is included.

With more time, I would add direct feeds or licensed APIs for Bloomberg, LCD, Creditflux, Private Debt Investor, Debtwire, SCI, ABL Advisor, SEC EDGAR company feeds, and manager IR pages; add an analyst review screen; add OpenAI-backed bilingual summarization with source citations; track recurring borrowers and funds; and store provenance / relevance scores per item for auditability.
